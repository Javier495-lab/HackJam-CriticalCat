...
public class CustomCursorHandler : MonoBehaviour
{
	...
	private Vector3 initialPrefabWorldPosition;
	...
	private void OnMouseDown()
	{
		...
		initialPrefabWorldPosition = transform.position;
	...
	{
		ResetCursor();
		LevelManager.Instance.ReturnHand();
	}
	...
	private void UpdateCustomCursorPosition()
	{
		...
		Vector3 prefabMovementSinceClick = transform.position - initialPrefabWorldPosition;
		Vector3 handPos = new Vector3(
						customCursorRenderer.transform.position.x - prefabMovementSinceClick.x,
						customCursorRenderer.transform.position.y,
						0);
		LevelManager.Instance.MoveHand(handPos);
	}
